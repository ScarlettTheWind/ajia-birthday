...
function selectYear(index){
  const direction=index>=previousYear?1:-1;previousYear=index;
  ...
  const image=node('div','year-photo');image.append(yearPhoto(y));const body=node('div','year-story-text');body.append(node('span','eyebrow',`${y[0]} · ${y[4]||'我们的大学时光'}`));
  y[2].split('\n').filter(Boolean).forEach(p=>body.append(node('p','',p)));story.replaceChildren(image,body);
  if(!matchMedia('(prefers-reduced-motion: reduce)').matches)story.animate([{opacity:.2,translate:`${direction*24}px 0`,scale:'.985'},{opacity:1,translate:'0 0',scale:'1'}],{duration:500,easing:'cubic-bezier(.16,1,.3,1)'});
}
config.years.forEach((y,i)=>{const frame=node('button','film-frame');frame.id=`year-tab-${i}`;frame.setAttribute('role','tab');frame.setAttribute('aria-controls','year-story');frame.append(node('span','film-year',`${String(i+1).padStart(2,'0')} / ${y[0]}`),yearPhoto(y));frame.onclick=()=>selectYear(i);frame.onkeydown=e=>{let next=i;if(e.key==='ArrowRight')next=(i+1)%frames.length;else if(e.key==='ArrowLeft')next=(i+frames.length-1)%frames.length;else if(e.key==='Home')next=0;else if(e.key==='End')next=frames.length-1;else return;e.preventDefault();selectYear(next);frames[next].focus({preventScroll:true});frames[next].scrollIntoView({block:'nearest',inline:'nearest',behavior:'instant'});};frames.push(frame);film.append(frame);});
...
const revealed=new Set();config.wishes.forEach((wish,i)=>{
  const b=node('button','wish');const inner=node('span','wish-inner');const front=node('span','wish-face wish-front');const back=node('span','wish-face wish-back',wish);
  front.append(node('span','number',String(i+1).padStart(2,'0')),node('small','','点开小祝愿 ♡'));back.setAttribute('aria-hidden','true');inner.append(front,back);b.append(inner);
  b.setAttribute('aria-label',`第${i+1}个祝愿，点击翻开`);b.setAttribute('aria-pressed','false');
  b.onclick=()=>{const on=b.classList.toggle('revealed');b.setAttribute('aria-pressed',String(on));b.setAttribute('aria-label',on?wish:`第${i+1}个祝愿，点击翻开`);front.setAttribute('aria-hidden',String(on));back.setAttribute('aria-hidden',String(!on));if(on)revealed.add(i);$('wish-count').textContent=`${revealed.size} / 24`;if(revealed.size===config.wishes.length&&!rewardShown){rewardShown=true;setTimeout(()=>wishReward.showModal(),matchMedia('(prefers-reduced-motion: reduce)').matches?0:650);}};
  $('wish-grid').append(b);
});
...
const heartCurtain=node('div','heart-curtain');heartCurtain.setAttribute('aria-hidden','true');document.body.append(heartCurtain);
...
// Ending reel: one accessible set plus a visual-only copy for seamless looping.
...
function openReel(index){reelLarge.src=config.endingPhotos[index];reelLarge.alt=`回忆照片 ${index+1}`;reelDialog.showModal();reelState();}
config.endingPhotos.forEach((src,i)=>{const button=node('button','reel-photo');button.type='button';button.dataset.photo=String(i);button.setAttribute('aria-label',`放大回忆照片 ${i+1}`);const img=node('img');img.src=src;img.alt=`回忆照片 ${i+1}`;img.loading='lazy';img.decoding='async';button.append(img,node('span','',String(i+1).padStart(2,'0')+' / MEMORIES'));reelGroup.append(button);});
const reelCopy=reelGroup.cloneNode(true);reelCopy.setAttribute('aria-hidden','true');reelCopy.querySelectorAll('button').forEach(b=>b.tabIndex=-1);reelTrack.append(reelGroup,reelCopy);reelViewport.append(reelTrack);reel.append(reelToggle,reelViewport,node('p','reel-last-line','照片会绕回来，我们的故事会继续。'));$('closing').after(reel);
reelToggle.onclick=()=>{reelManual=!reelManual;reelState();};
reelViewport.addEventListener('click',e=>{const b=e.target.closest('[data-photo]');if(b)openReel(Number(b.dataset.photo));});
reelViewport.addEventListener('pointerenter',e=>{if(e.pointerType==='mouse'){reelHover=true;reelState();}});reelViewport.addEventListener('pointerleave',()=>{reelHover=false;reelState();});
reelViewport.addEventListener('pointerdown',e=>{if(e.pointerType!=='mouse'){reelTouch=true;reelState();}},{passive:true});
for(const event of ['pointerup','pointercancel'])window.addEventListener(event,()=>{reelTouch=false;reelState();},{passive:true});
